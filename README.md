...
The default view is intentionally concise and technically precise without assuming prior MCP expertise. It retains terms such as agent client, MCP server, tool, capability, attack chain, least privilege, classification coverage, and security posture, then explains their consequence or definition where they appear. Each high-priority finding identifies the client, server, tool, capability, reason, and recommended control. Supporting findings and complete evidence remain in the local HTML report. The CLI writes that report without opening it; use `--open-report` when you explicitly want a browser window.

The guided inspection targets about 78 seconds from launch and walks through 14 evidence-backed phases. Each phase reveals a result derived from the audit—inventory availability, schema coverage, sensitive surfaces, direct authority, cross-server paths, role overlap, unknown tools, and final confidence—while keeping the terminal to one changing status line. After analysis, the report appears progressively in small word groups with short sentence and section pauses, similar to model token generation; it is not held and dumped all at once. The paced inspection and progressive reveal run only in an interactive terminal. JSON output, piped output, CI, `--yes`, `--quick`, `--no-motion`, and `SLAUNT_NO_MOTION=1` remain immediate. The audit also refuses to start a configured stdio entry that resolves back to the `slaunt` CLI, preventing recursive audit windows. The experience deliberately avoids variable rewards, deceptive urgency, hidden defaults, shame, or an unsupported claim that a UI can medically “optimize dopamine.”
