...
import type { AuditResult, Capability, Severity } from '../types.js';
...
const capabilityLabels: Record<Capability, string> = {
  ...
  'network-access': 'Access network', 'deploy-production': 'Deploy production', 'edit-code': 'Edit code',
...
function accessOverview(result: AuditResult): string {
  return result.discovery.clients.filter((client) => client.detected).map((client) => {
    ...
    const rows = servers.map((server) => {
      ...
      return `<tr><td>${escapeHtml(server.name)}</td><td>${escapeHtml(capabilities.map((item) => capabilityLabels[item]).join(', ') || 'Tool list unavailable')}</td></tr>`;
    }).join('');
...
export function renderHtmlReport(result: AuditResult): string {
  ...
  const findings = result.findings.map((item) => `
    <article class="finding ${escapeHtml(item.severity)}">
      <div class="severity">${severityLabels[item.severity]}</div>
      <h3>${escapeHtml(item.title)}</h3>
      ${item.serverNames.length ? `<p><strong>Servers:</strong> ${escapeHtml(item.serverNames.join(', '))}</p>` : ''}
      ${item.toolNames.length ? `<p><strong>Tools:</strong> ${escapeHtml(item.toolNames.join(', '))}</p>` : ''}
      <p><strong>Why flagged:</strong> ${escapeHtml(item.why)}</p>
  ...
  const scoreClass = summary.postureScore >= 80 ? 'good' : summary.postureScore >= 55 ? 'warn' : 'bad';

  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src data:">
<title>Slaunt Agent Access Audit</title><style>
...
<header><div><div class="eyebrow">Slaunt audit</div><h1>Agent access, mapped.</h1><p class="sub">A local-first inventory of MCP tools exposed to Claude and Codex. No tool was called and no audit payload was uploaded.</p></div><div class="score ${scoreClass}" style="--score:${summary.postureScore}"><span>${summary.postureScore}<small>posture</small></span></div></header>
<section class="metrics">
<div class="metric"><b>${summary.clientsDetected}</b><span>Clients</span></div><div class="metric"><b>${summary.servers}</b><span>MCP servers</span></div><div class="metric"><b>${summary.tools}</b><span>Tools exposed</span></div><div class="metric"><b>${summary.sensitiveCapabilities}</b><span>Sensitive tools</span></div><div class="metric"><b>${summary.highRiskFindings}</b><span>High-risk findings</span></div><div class="metric"><b>${summary.unclassified}</b><span>Unclassified</span></div>
</section>
<div class="notice">Classification catalog: <strong>${escapeHtml(result.privacy.catalogSource)}</strong>. Tool names, descriptions, schemas, configuration values, and findings remained on this machine.</div>
<h2>Needs attention</h2>${findings || '<p>No correlated high-risk paths were identified from the available declarations.</p>'}
<h2>Access overview</h2><div class="grid">${accessOverview(result)}</div>
<h2>Classification coverage</h2><p><strong>${summary.classified}</strong> verified or recognized · <strong>${summary.unclassified}</strong> unknown</p>
${unknown.length ? `<ul class="unknown">${unknown.map((tool) => `<li><code>${escapeHtml(tool.name)}</code> <span>(${escapeHtml(tool.serverName)})</span></li>`).join('')}</ul>` : '<p>Every retrieved tool matched a catalog rule.</p>'}
<section class="cta"><h2>You found the paths. Put guardrails on them.</h2><p>Slaunt can add role-scoped access, approvals, continuous monitoring, and remote shutdown around the MCP stack you already use. You choose when to connect.</p><a href="https://slaunt.ai/setup">Create a Slaunt workspace →</a></section>
<p class="fine">Scope: configured MCP servers and their declared tools only. This audit did not inspect prompts, file contents, credentials, non-MCP agents, or whether any tool has been used. Generated ${escapeHtml(result.generatedAt)}.</p>
</main></body></html>`;
}
