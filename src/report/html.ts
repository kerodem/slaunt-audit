...
export function renderHtmlReport(result: AuditResult): string {
  ...
  const findings = result.findings.map((item) => {
    ...
    <article class="finding ${escapeHtml(item.severity)}">
      ...
      ${assistants ? `<p><strong>AI assistant:</strong> ${escapeHtml(assistants)}</p>` : ''}
      ${item.serverNames.length ? `<p><strong>Connected service:</strong> ${escapeHtml(item.serverNames.join(', '))}</p>` : ''}
      ${item.toolNames.length ? `<p><strong>Technical action name:</strong> ${escapeHtml(item.toolNames.join(', '))}</p>` : ''}
      <p><strong>Why this matters:</strong> ${escapeHtml(copy.why)}</p>
      <p><strong>What you can do:</strong> ${escapeHtml(copy.recommendation)}</p>
    </article>`;
  }).join('');
...
<title>Slaunt AI Assistant Safety Check</title><style>
...
<header><div><div class="eyebrow">Slaunt safety check</div><h1>See what your AI assistants can do.</h1><p class="sub">Slaunt reviewed the actions available to Claude and Codex on this computer. It did not use those actions, and no audit payload was uploaded.</p></div><div class="score ${scoreClass}" style="--score:${scoreValue}"><span>${scoreAvailable ? summary.postureScore : '?'}<small>${scoreAvailable ? 'safety score' : 'not enough data'}</small></span></div></header>
<section class="metrics">
<div class="metric"><b>${summary.clientsDetected}</b><span>AI assistants checked</span></div><div class="metric"><b>${summary.servers}</b><span>Connected services</span></div><div class="metric"><b>${summary.tools}</b><span>Available actions</span></div><div class="metric"><b>${summary.sensitiveCapabilities}</b><span>Sensitive actions</span></div><div class="metric"><b>${summary.highRiskFindings}</b><span>Urgent issues</span></div><div class="metric"><b>${summary.unclassified}</b><span>Need review</span></div>
</section>
<div class="notice">Privacy: Slaunt used its <strong>${escapeHtml(result.privacy.catalogSource)}</strong> safety guide. Your connected services, available actions, settings, and results stayed on this computer.</div>
<h2>What needs your attention</h2>${findings || '<p>No urgent access issues were found in the information Slaunt could read.</p>'}
<h2>What each assistant can do</h2><div class="grid">${accessOverview(result)}</div>
<h2>Items Slaunt could not identify</h2><p>Slaunt understood <strong>${summary.classified}</strong> actions. <strong>${summary.unclassified}</strong> need a person to review them.</p>
${unknown.length ? `<ul class="unknown">${unknown.map((tool) => `<li><code>${escapeHtml(tool.name)}</code> <span>(${escapeHtml(tool.serverName)})</span></li>`).join('')}</ul>` : '<p>Slaunt recognized every available action.</p>'}
<section class="cta"><h2>Want ongoing protection?</h2><p>Add approval prompts, give each assistant only the access it needs, monitor changes, and turn off access remotely when necessary. You choose when to connect.</p><a href="https://slaunt.ai/setup">Set up Slaunt →</a></section>
<p class="fine">What this check covered: actions advertised by connected services. Slaunt did not use those actions or read prompts, files, credentials, other types of AI assistants, or past activity. Generated ${escapeHtml(result.generatedAt)}.</p>
</main></body></html>`;
}
